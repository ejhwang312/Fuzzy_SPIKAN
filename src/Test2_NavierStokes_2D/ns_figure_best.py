...
def plot_centerline_comparison(solutions, reference_fvm_data, Re):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, f'centerline_comparison_Re{Re}_dpi_150.png'), dpi=150, bbox_inches='tight')
    plt.show()
...
def plot_field_comparison(solutions, reference_fvm_data, Re):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, f'field_comparison_Re{Re}_dpi_150.png'), dpi=150, bbox_inches='tight')
    plt.show()
...
def plot_absolute_difference(solutions, reference_fvm_data, Re):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, f'absolute_difference_comparison_Re{Re}_dpi_150.png'), dpi=150, bbox_inches='tight')
    plt.show()
...
if __name__ == "__main__":
    ...
    if len(solutions) > 0:
        ...
        filtered_solutions = {k: v for k, v in solutions.items() if k in ['spikan', 'or-spikan']}

        plot_field_comparison(filtered_solutions, reference_fvm_data, Re)

        plot_absolute_difference(filtered_solutions, reference_fvm_data, Re)
